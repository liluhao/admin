...
funcBuildServer() {
    ...
    cd $serverDir
    go mod tidy
    GOOS=linux GOARCH=amd64 go build -o $serverDir/admin
    if [  -f $serverDir/admin ]; then
...
funcDockerInitMysqlAndRedis(){
  ...
  funcStartServer(){
    ...
    cd  $serverDir
     command=`netstat -nlp |grep admin| awk '{print $4}'| awk -F":" '{ print $4 }'`
   if [  "$command" == "" ]; then
     nohup ./admin &
     sleep 1
     echo -e  'server start  success' ;
     local port="`netstat -nlp |grep admin| awk '{print $4}'| awk -F":" '{ print $4 }'`";
     echo "listen port:$port"
   else
     ps -ef |grep admin|grep -v grep|awk '{print $2}'|xargs kill -9
     nohup ./admin &
     sleep 1
	   echo -e  'server start  success' ;
     local port="`netstat -nlp |grep admin| awk '{print $4}'| awk -F":" '{ print $4 }'`";
